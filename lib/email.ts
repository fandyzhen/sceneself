...
const getDefaultFromEmail = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    return 'SceneSelf <onboarding@resend.dev>';
  }
  ...
  // 4. 使用配置的域名和应用名称
  const appName = process.env.NEXT_PUBLIC_APP_NAME || 'SceneSelf';
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="color: #999; font-size: 14px; margin-top: 30px;">
          If you didn't sign up for SceneSelf, you can safely ignore this email.
        </p>
      </div>
    `,
  });
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  return sendEmail({
    to: email,
    subject: 'Reset your password - SceneSelf',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
...
export async function sendWelcomeEmail(email: string, name?: string) {
  return sendEmail({
    to: email,
    subject: 'Welcome to SceneSelf!',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #333;">Welcome to SceneSelf${name ? ', ' + name : ''}!</h1>
...
export async function sendPurchaseEmail(email: string, orderDetails: PurchaseEmailDetails) {
  // 三档订阅命名（weekly/monthly/yearly）。one_time 已下线，留为兜底显示。
  const planName = orderDetails.type === 'subscription'
    ? `${orderDetails.plan} subscription`
    : `${orderDetails.credits} credits`;

  return sendEmail({
    to: email,
    subject: 'Purchase Confirmation - SceneSelf',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          ${orderDetails.type === 'subscription' ? '<p><strong>Type:</strong> Recurring subscription</p>' : ''}
        </div>
        <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard" style="display: inline-block; padding: 12px 24px; background-color: #000; color: #fff; text-decoration: none; border-radius: 6px; margin: 20px 0;">
          View Dashboard
        </a>
        <p style="color: #999; font-size: 14px; margin-top: 30px;">
          Thank you for choosing SceneSelf!
        </p>
      </div>
    `,
  });
...
export async function sendSubscriptionExpiryReminder(email: string, daysRemaining: number) {
  return sendEmail({
    to: email,
    subject: `Your subscription expires in ${daysRemaining} days - SceneSelf`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #333;">Subscription Expiry Reminder</h1>
        <p>Your SceneSelf subscription will expire in <strong>${daysRemaining} days</strong>.</p>
...
export async function sendLowCreditsNotification(email: string, remainingCredits: number) {
  return sendEmail({
    to: email,
    subject: 'Low Credits Alert - SceneSelf',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
