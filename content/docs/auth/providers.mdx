...
A verification email is sent on signup. Login does not require verification (`requireEmailVerification: false` in `lib/auth.ts`), but key actions such as saving and exporting scene sets are gated by `EmailVerifiedGuard`:
...
Google login is optional. SceneSelf only enables the provider when both `AUTH_GOOGLE_ID` and `AUTH_GOOGLE_SECRET` are configured.
