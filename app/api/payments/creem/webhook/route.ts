...
import { isSubscriptionKey, subscriptionPlans, PlanKey } from "@/constants/billing";
import {
  computeInitialGrant,
  getGrantSchedule,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (kind === "one_time") {
      // story-pack 已下线（SPEC v3.1）：理论上不应再收到 one_time webhook，
      // 即便兜底收到也直接 ack，避免误发积分或写入孤立 payment 行。
      return NextResponse.json({ received: true });
